...
import copy
import ros_numpy #cloned submodule
...
from open3d_resources.helper import R3D
...
class R3DROS(R3D):
    ...
    def __init__(self,
        ...
        self.pcd_frame_id = 0
        self.pub_pcd = rospy.Publisher(pcd_out_topic, PointCloud2, queue_size=10)
        

    # Callback        
    def _callback(self, rgb, depth):
        if rgb and depth:
            cv_rgb = self.image_transport(rgb)
            cv_depth = self.image_transport(depth)
            
            o3d_rgb = cv_rgb[...,::-1].copy()
            o3d_depth = cv_depth.astype(np.float32)
        
            o3d_pcd = self.generate_rgbd_point_cloud(o3d_rgb, o3d_depth)
            ros_pcd = self.o3dpc_to_rospc(o3d_pcd, self.pcd_frame_id)
